...
/*
 * Modified to interface with .C call from R and to return RMSD value.
 * Copyright (C) 2019, Jens Kleinjung (jens@jkleinj.eu)
*/
...
/* C wrapper for R calls to Kabsch routine */
/* 'Xarray' and 'Yarray' are coordinate array of 'size' atoms (points). */
/* They are cast into the GSL matrix format for the Kabsch routine below. */
void wrap_kabsch(double *size, double *Xarray, double *Yarray, double *rmsd)
{
	unsigned int i;
	unsigned int isize = (int)*size;

	/* allocate GSL objects for Kabsch routine */
	gsl_matrix *X = gsl_matrix_alloc(isize, 3); /* fragment to be moved */
	gsl_matrix *Y = gsl_matrix_alloc(isize, 3); /* fragment to move to */
	...
	/* cast R coordinate arrays into GSL matrix */
	/* expected format: 1x,1y,1z, 2x,2y,2z, 3x,3y,3z, 4x,4y,4z */
	for (i = 0; i < isize; ++ i) {
		/* X */
		gsl_matrix_set(X,i,0, Xarray[(3 * i)    ]);
		gsl_matrix_set(X,i,1, Xarray[(3 * i) + 1]);
		gsl_matrix_set(X,i,2, Xarray[(3 * i) + 2]);

		/* Y */
		gsl_matrix_set(Y,i,0, Yarray[(3 * i)    ]);
		gsl_matrix_set(Y,i,1, Yarray[(3 * i) + 1]);
		gsl_matrix_set(Y,i,2, Yarray[(3 * i) + 2]);
	}

	/* call Kabsch routine to superimpose fragments X and Y */
    kabsch(isize, X, Y, U, t, &s);
	...
	for (i = 0, varsum = 0.; i < isize; ++ i) {
		/* move X */
		xm_x =  Xarray[(3 * i)    ] * gsl_matrix_get(U,0,0) + 
				Xarray[(3 * i) + 1] * gsl_matrix_get(U,0,1) + 
				Xarray[(3 * i) + 2] * gsl_matrix_get(U,0,2) + 
				gsl_vector_get(t,0); 
 
		xm_y =	Xarray[(3 * i)    ] * gsl_matrix_get(U,1,0) + 
				Xarray[(3 * i) + 1] * gsl_matrix_get(U,1,1) + 
				Xarray[(3 * i) + 2] * gsl_matrix_get(U,1,2) + 
				gsl_vector_get(t,1); 
 
		xm_z =	Xarray[(3 * i)    ] * gsl_matrix_get(U,2,0) + 
			    Xarray[(3 * i) + 1] * gsl_matrix_get(U,2,1) + 
				Xarray[(3 * i) + 2] * gsl_matrix_get(U,2,2) + 
				gsl_vector_get(t,2); 

		/* calculate coordinate variance sum between moved X and Y */
		varsum +=  pow(xm_x - Yarray[(3 * i)    ], 2)
				 + pow(xm_y - Yarray[(3 * i) + 1], 2)
		 		 + pow(xm_z - Yarray[(3 * i) + 2], 2);
	}
